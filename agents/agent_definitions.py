...
# 1. Groq LLM (High-speed Llama 3.3 70B Versatile on Groq API for Extraction & Translation)
groq_llm = LLM(
...
# 3. OpenRouter Guardrail LLM (Llama 3.1 8B Instruct for Compliance Auditing)
openrouter_guardrail = LLM(
    model="openrouter/meta-llama/llama-3.1-8b-instruct:free",
    api_key=openrouter_api_key if openrouter_api_key else groq_api_key,
    base_url="https://openrouter.ai/api/v1" if openrouter_api_key else None
)

# 1. Extraction Agent -> Powered by Groq (Ultra-fast Llama 3.3 70B parsing)
...
analyzer_agent: Agent = Agent(
    ...
    tools=[get_rag_tool()],
    llm=openrouter_primary,
    verbose=True
)

# 3. Plain-English Translator Agent -> Powered by Groq (Llama 3.3 70B multi-lingual guidance in EN, Sinhala, Tamil)
...
# 4. Clinical Guardrail Agent (Reviewer) -> Powered by OpenRouter (Llama 3.1 8B for independent safety compliance auditing)
